...
class IteratorTools
{
     ...
     *     с помощью перебора, для {@see Traversable} объектов это может быть опасно, например, если итератор
     ...
     *
     *  (!) {@see \Generator} позволяет перебрать его только один раз, клонировать генератор в невозможно, поэтому посчитать
     *      count() для генератора, который не реализует {@see \Countable} невозможно
     *
     * См также:
    ...
    {
        // (!) Входящий тип для $iterable нельзя ставить iterable|\Countable, так как объекты базово не являются "перебираемым"
